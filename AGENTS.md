...
- If you change session lifecycle or transitions, start in `SessionStore`. Avoid ad-hoc state mutation elsewhere.
  - Workspace supersession and liveness cleanup must protect `needsPromptNotification`, including terminal-only questions with suppressed in-app controls; `needsManualAttention` only covers Island-answerable requests. Explicit SessionEnd still takes precedence. Hover previews keep connected waiting-for-input and recently completed sessions, while disconnected rows remain outside the active preview.
...
- If you change completion-result popup behavior, trace through `SessionStore`, `SessionMonitor`, `PingIsland/UI/Views/NotchView.swift`, and `PingIsland/UI/Views/SessionCompletionNotificationView.swift` so completion detection, queueing, and auto-dismiss timing stay aligned.
  - Completion side effects use `SessionCompletionKey`: a provider `latestTurnId` alone identifies its turn; `completionSequence` is only a fallback when no turn ID exists. Do not combine the sequence with a provider turn ID or final assistant item identity, since replayed running snapshots and late transcript details must not repeat a popup or sound. The single sound edge tracker is owned by `SessionMonitor`; do not reintroduce timestamp-based deduplication or docked/detached view-local sound trackers that reset during surface changes.
  - Transcript enrichment never owns `autoApprovePermissions`. Always retain the Store's current setting when committing enriched content, including a toggle with no phase change and the reset performed by SessionEnd.
  - Codex abort evidence is retained in `isCodexTurnInterrupted`: idle refreshes and stale processing snapshots of the same turn must not announce success or clear it. New processing activity or a different provider turn ID clears it; successful completion does not require transcript text.
  - Compaction notifications use the Store's independent `compactionSequence` plus the row incarnation. Advance only when entering `.compacting`, preserve it through transcript enrichment and row rebuilds, and keep it separate from completed-turn keys so multiple compactions in one turn remain visible without duplicate surface notifications.
...
- If you change Codex behavior, verify both the monitor layer under `PingIsland/Services/Codex/` and the UI under `PingIsland/UI/Views/CodexSessionView.swift`.
  - A background `thread/read` must retain the actual pending server request, not replace it with transcript inference. Carry the pre-read intervention into the Store commit so a request or resolution arriving across either await boundary keeps its current card and phase. Regression tests must execute the read/parse/commit path, not only disconnected notification handlers.
